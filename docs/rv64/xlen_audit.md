...
- **`hw/rtl/cpu_and_mem/cpu/riscv_pkg.sv`** [resolved] The planned RV64 instruction-operation members have landed. ZIP and UNZIP were later removed with RV32 support, but their former ordinals 86 and 87 remain reserved so the 119 live operations after them keep their established encodings through ordinal 206. `instr_op_e` has an explicit unsigned, two-state `bit [7:0]` base instead of the implicit 32-bit `int`, preserving the packed carrier width without perturbing the synthesized operation decode.
  - *Resolution:* `InstrOpWidth=8` is shared by the RS payload and verification mirrors; enum-parsing benches accept both implicit and explicit assignments as well as bit/logic bases.
...
- `hw/rtl/cpu_and_mem/cpu/if_stage/branch_prediction/branch_prediction_controller.sv:522` [low] pd_redirect_kills_prediction_metadata compares o_predicted_target_r != i_pd_redirect_target — widens to a 64-bit inequality feeding the synchronous-clear terms of three holdoff/metadata flop groups (lines 541-653). o_predicted_target_r is a raw flop; i_pd_redirect_target arrives one shallow post-register format/bank select LUT after PD's split target flops, and the compare fans out into control logic the comments identify as timing-sensitive.
...
compute their own low 13-bit sum and raw two-bit `{sign, carry}` state.
The late format select chooses only those 15 bits before the existing nonstall
redirect-register edge. That edge separately captures the selected low/state
...
reaches only the 15 low/state D inputs, and the post-boundary target high path is
