...
static void test_regression(void)
{
    ...
    TFP("f large round", "1000000.000000", 14, "%f", 999999.9999996);
    ...
    TFP("g 1e5 fixed", "100000", 6, "%g", 100000.0);
...
{
    section("Float range");
    T("e 1e-19", "1.000000e-19", 12, "%e", 1e-19);
    T("e 1e-30", "1.000000e-30", 12, "%e", 1e-30);
    T("g 1e-30", "1e-30", 5, "%g", 1e-30);
    T("e 1e-300", "1.000000e-300", 13, "%e", 1e-300);
    T("e DBL_MIN", "2.225074e-308", 13, "%e", 2.2250738585072014e-308);
    T("e min subnormal", "4.940656e-324", 13, "%e", 4.9406564584124654e-324);
    T(".17g min subnormal", "4.9406564584124654e-324", 23, "%.17g", 4.9406564584124654e-324);
    T("e DBL_MAX", "1.797693e+308", 13, "%e", 1.7976931348623157e308);
    T("f 1e20", "100000000000000000000.000000", 28, "%f", 1e20);
    T("f 2^64", "18446744073709551616.000000", 27, "%f", 18446744073709551616.0);
    T("f 1e23", "99999999999999991611392.000000", 30, "%f", 1e23);
    T(".40g 1e35", "99999999999999996863366107917975552", 35, "%.40g", 1e35);
    T("f 15 digits", "123456789012345.000000", 22, "%f", 123456789012345.0);
    T("f 2^52+1", "4503599627370497.000000", 23, "%f", 4503599627370497.0);
    T(".20f 0.1", "0.10000000000000000555", 22, "%.20f", 0.1);
    T(".30e 1/3", "3.333333333333333148296162562474e-01", 36, "%.30e", 1.0 / 3.0);
    {
        /* DBL_MAX has 309 integer digits. */
        char got[320];
        int r = snprintf(got, sizeof(got), "%.0f", 1.7976931348623157e308);
        check(".0f DBL_MAX",
              "17976931348623157081452742373170435679807056752584499659891747680315726078002853876"
              "05895586327668781715404589535143824642343213268894641827684675467035375169860499105"
              "76551282076245490090389328944075868508455133942304583236903222948165808559332123348"
...
static void test_float_rounding(void)
{
    ...
    T(".0f 1.5", "2", 1, "%.0f", 1.5);
    T(".0f 2.5", "2", 1, "%.0f", 2.5);
    T(".2f 0.125", "0.12", 4, "%.2f", 0.125);
    T(".2f 0.375", "0.38", 4, "%.2f", 0.375);
    T(".1e 1.25", "1.2e+00", 7, "%.1e", 1.25);
    T(".0e 9.5", "1e+01", 5, "%.0e", 9.5);
    T("f below tie", "999999.999999", 13, "%f", 999999.9999995);
    T(".3f above tie", "1000.000", 8, "%.3f", 999.9995);
    T("g carry to e", "1e+06", 5, "%g", 999999.5);
    T("g carry to f", "0.0001", 6, "%g", 0.000099999995);
    T("g carry to 10", "10", 2, "%g", 9.9999995);
    /* '#' keeps the trailing zeros after a carry too (C11 7.21.6.1). */
    T("#g carry", "1.00000e+06", 11, "%#g", 999999.5);
...
static void test_float_special(void)
{
    ...
    T("G nan", "NAN", 3, "%G", nan);
    T("e nan", "nan", 3, "%e", nan);
    T("f negative nan", "-nan", 4, "%f", -nan);
    T("plus inf", "+inf", 4, "%+f", inf);
    T("space inf", " inf", 4, "% e", inf);
    T("plus nan", "+nan", 4, "%+g", nan);
    T("inf width", "   inf", 6, "%6f", inf);
    T("inf zero flag", "  -inf", 6, "%06f", -inf);
