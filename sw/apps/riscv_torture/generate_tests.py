...
Usage:
    ...
    ./generate_tests.py --references-only --test tests_rv64/test_001.S
"""
...
# A atomics (word and doubleword forms)
AMO_OPS = [
    "amoadd.w",
    "amoswap.w",
    "amoand.w",
...
def _rand_shamt(rng: random.Random) -> int:
    """Shift amount for the full-width 64-bit shifts (0-63)."""
    return rng.randint(0, 63)


def _gen_alu_seq(rng: random.Random, lines: list[str]) -> None:
    """Generate 1-3 random ALU instructions (W-forms mixed in)."""
    for _ in range(rng.randint(1, 3)):
        use_w = rng.random() < 0.35
        if rng.random() < 0.5:
...
def _gen_mul_seq(rng: random.Random, lines: list[str]) -> None:
    """Generate 1-2 random multiply/divide instructions (W-forms mixed in)."""
...
def _gen_mem_seq(rng: random.Random, lines: list[str], memsize: int) -> None:
    ...
    # double is LD/SD (8-byte aligned); wordu is the zero-extending LWU.
    op_types = ["word", "half", "byte", "double", "wordu"]
...
def _gen_amo_seq(rng: random.Random, lines: list[str], memsize: int) -> None:
    """Generate a single AMO instruction (word and .d forms)."""
    ...
    # of the compared GPR set (see COMPUTE_GPRS).
    addr_reg = AMO_ADDR_REG
...
def generate_test(seed: int, nseqs: int = 200, memsize: int = 1024) -> str:
    """Generate a random RV64 IMAFDC torture test."""
...
def main() -> int:
    ...
    if args.generate:
        ...
        print(
            f"Generating {args.count} rv64 torture tests "
