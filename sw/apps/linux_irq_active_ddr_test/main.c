...
#define IRQ_COUNT (NORMAL_IRQ_COUNT + POISON_IRQ_COUNT + ACTIVE_IRQ_COUNT)
...
__attribute__((noinline, used)) void linux_like_irq_c(struct linux_pt_regs *frame)
{
    ...
    if (frame->cause != (MCAUSE_INTERRUPT_BIT | INT_MTI)) {
        ...
        uart_printf("FAIL code=%u ticks=%u cause=%08x mepc=%08x ra=%08x\n",
                    g_fail_code, g_ticks, frame->cause, frame->epc, frame->ra);
        uart_printf("<<FAIL>>\n");
        for (;;) {
        }
    }
...
__attribute__((noreturn, noinline, used)) void main_on_ddr_stack(void)
{
    ...
    if (!g_fail_seen && g_ticks == IRQ_COUNT && aggregate != 0u) {
        ...
        uart_printf("FAIL code=%u ticks=%u cause=%08x mepc=%08x ra=%08x sp=%08x tp=%08x mscratch=%08x\n",
                    g_fail_code, g_ticks, g_bad_cause, g_last_mepc, g_last_ra,
                    g_last_sp, g_last_tp, g_last_mscratch_in_handler);
        uart_printf("<<FAIL>>\n");
    }
