...
/* Allocate `size` bytes on arena. 16 byte aligned. Returns NULL if out of space. */
void *arena_push(arena_t *arena, uint32_t size);
/* Allocate `size` bytes on arena and zero the data. 16 byte aligned. */
...
/* Empty the arena. Its memory is not zeroed. */
void arena_clear(arena_t *arena);

/* Allocate 16-byte-aligned storage. Zero-size, overflowing, and out-of-heap
