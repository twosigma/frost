...
The generic platform uses ``opensbi_frost_defconfig``, firmware at 0x80000000,
and the payload at +2 MiB. An empty FW_JUMP_FDT_OFFSET preserves the packer's
DTB pointer in a1. FDT_ASSUME_MASK=7 trusts the DTB generated by the packer/dtc.
The submodule is unmodified; all settings use OpenSBI Make variables.

Outputs: <out>/platform/generic/firmware/fw_jump.{bin,elf}.
Uses Bootlin with PIE; FROST_LINUX_CROSS_COMPILE or --cross overrides the prefix.
"""
