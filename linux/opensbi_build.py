...
v1.9), built for the FROST boot layout:
...
* ``platform-cflags-y=-DFDT_ASSUME_MASK=7`` (libfdt ASSUME_VALID_DTB |
  ...
  pure overhead. With OpenSBI 1.7, device-tree walks
  (fdt_offset_ptr/fdt_next_tag) were ~80% of its boot instructions; together
  with the slim defconfig this cut firmware entry through final platform
  init from ~3.2M to ~1.7M instructions on Spike. Those are historical
  measurements, before the OpenSBI 1.9 upgrade.
