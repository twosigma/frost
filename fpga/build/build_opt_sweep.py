...
"""Run FPGA opt_design with multiple directives in parallel and report results.

This script runs synthesis once (or uses an existing post_synth checkpoint), then
forks into parallel opt_design runs with different directives. Results are reported
for comparison - this is an exploration tool to find the best opt_design directive.
"""
...
# All available Vivado opt_design directives (case-sensitive)
# Note: RQS requires an RQS file, so we exclude it from the default list
ALL_OPT_DIRECTIVES = [
    "Default",
    "Explore",
    "ExploreArea",
...
@dataclass
class OptResult:
    """Optimization results from a Vivado run."""
...
def select_best_result(results: list[OptResult]) -> OptResult | None:
    """Select the best result based on timing.

    Priority:
    1. Best WNS (highest value = least negative = best slack)
    2. Best TNS (least negative total negative slack)
    """
    # Filter out runs that failed (no timing data)
    valid_results = [r for r in results if r.wns_ns is not None and r.success]

    if not valid_results:
        return None

    # Sort by: WNS (highest first), then TNS (highest first)
...
def move_winner_to_main_work(
    ...
    Also preserves post_synth files from original work directory.
    """
    main_work = script_dir / board_name / "work"

    # Preserve post_synth files from original work directory before removing it
    post_synth_dcp = main_work / "post_synth.dcp" if main_work.exists() else None
    post_synth_timing = (
        main_work / "post_synth_timing.rpt" if main_work.exists() else None
    )
    post_synth_util = main_work / "post_synth_util.rpt" if main_work.exists() else None
    post_synth_fanout = (
        main_work / "post_synth_high_fanout.rpt" if main_work.exists() else None
    )
    post_synth_paths = (
        main_work / "post_synth_failing_paths.csv" if main_work.exists() else None
    )
    ...
    for f in [
        ...
        if f and f.exists():
            synth_files_to_copy.append(f)

    # Remove main work directory if it exists
    if main_work.exists():
        # First copy synth files to a temp location
        temp_dir = script_dir / board_name / "work_synth_temp"
        temp_dir.mkdir(exist_ok=True)
        for f in synth_files_to_copy:
...
def run_synthesis(
    ...
    """Run synthesis only, return path to checkpoint."""
    ...
    checkpoint = work_dir / "post_synth.dcp"
...
def start_opt_process(
    ...
    """Start an opt_design process with a specific directive. Returns VivadoProcess."""
    vivado_command = [
        vivado_path,
        "-mode",
        "batch",
        ...
        str(checkpoint_path),  # checkpoint_path (post_synth.dcp)
        directive,  # work_suffix
        "PerformanceOptimized",  # synth_directive (not used when loading checkpoint)
...
def extract_utilization_from_report(util_rpt_path: Path) -> dict:
    ...
    # Extract CLB LUTs (UltraScale+) or Slice LUTs (7-series)
    lut_pattern = r"\|\s*(?:CLB|Slice) LUTs\*?\s*\|\s*(\d+)\s*\|"
    match = re.search(lut_pattern, util_rpt)
    if match:
        result["lut_count"] = int(match.group(1))

    # Extract CLB Registers (FFs)
    ff_pattern = r"\|\s*(?:CLB|Slice) Registers\s*\|\s*(\d+)\s*\|"
...
def harvest_results(
    ...
    for directive in directives:
        ...
        util_rpt = work_dir / "post_opt_util.rpt"
...
def print_results_table(results: list[OptResult], winner: OptResult | None) -> None:
    """Print a formatted table of results."""
    print("\n" + "=" * 128)
    print("OPT_DESIGN DIRECTIVE SWEEP RESULTS")
    print("=" * 128)
    print(
        f"{'Directive':<24} {'WNS (ns)':>10} {'TNS (ns)':>12} {'LUTs':>8} {'FFs':>8} {'BRAM':>6} {'DSP':>5} {'Status':>8} {'Winner':>8}"
    )
    print("-" * 128)
    ...
    for r in sorted(results, key=lambda x: x.wns_ns or float("-inf"), reverse=True):
        ...
        print(
            f"{r.directive:<24} {wns:>10} {tns:>12} {luts:>8} {ffs:>8} {bram:>6} {dsp:>5} {status:>8} {is_winner:>8}"
        )
    ...
    for r in area_sorted:
        print(f"  {r.directive:<24} {r.lut_count:>8} LUTs")


def main() -> None:
    """Run opt_design directive sweep."""
    parser = argparse.ArgumentParser(
        description="Run FPGA opt_design with multiple directives in parallel"
    )
    parser.add_argument(
        "board_name",
        nargs="?",
    ...
    parser.add_argument(
        "--directives",
        nargs="+",
        help="Specific directives to run (default: all opt_design directives)",
    )
    parser.add_argument(
        "--retiming",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-synth",
        action="store_true",
        help="Skip synthesis, use existing post_synth.dcp checkpoint",
    )
    parser.add_argument(
        "--keep-all",
        action="store_true",
    ...
    if args.directives:
        ...
        directives = ALL_OPT_DIRECTIVES.copy()
    ...
    # Step 2: Run synthesis (or use existing checkpoint)
    if args.skip_synth:
        checkpoint = script_dir / board_name / "work" / "post_synth.dcp"
        if not checkpoint.exists():
            print(f"Error: Checkpoint not found: {checkpoint}", file=sys.stderr)
            print("Run without --skip-synth first to generate it.")
            sys.exit(1)
        print(f"\nUsing existing checkpoint: {checkpoint}")
    else:
        checkpoint = run_synthesis(
            script_dir, board_name, args.retiming, args.vivado_path
        )
        if checkpoint is None:
            sys.exit(1)

    # Step 3: Clean up any existing work directories for these directives
    for directive in directives:
        work_dir = script_dir / board_name / f"work_{directive}"
        if work_dir.exists():
            shutil.rmtree(work_dir)

    # Step 4: Run opt_design with all directives in parallel
    print(f"\n{'='*60}")
    print(f"Running {len(directives)} opt_design jobs in parallel...")
    print(f"{'='*60}\n")
    ...
    # Harvest all results and select winner
    ...
    if winner:
        ...
        print(f"\nNext step: ./build_placer_sweep.py {board_name} --skip-synth-opt")
    else:
