...
def select_best_result(results: list[SynthResult]) -> SynthResult | None:
...
def print_results_table(results: list[SynthResult], winner: SynthResult | None) -> None:
    """Print a formatted table of results."""
    print("\n" + "=" * 128)
    print("SYNTHESIS DIRECTIVE SWEEP RESULTS")
    print("=" * 128)
    print(
        f"{'Directive':<28} {'WNS (ns)':>10} {'TNS (ns)':>12} {'LUTs':>8} {'FFs':>8} {'BRAM':>6} {'DSP':>5} {'Status':>8} {'Winner':>8}"
    )
    print("-" * 128)
    ...
    for r in sorted(results, key=lambda x: x.wns_ns or float("-inf"), reverse=True):
        ...
        print(
            f"{r.directive:<28} {wns:>10} {tns:>12} {luts:>8} {ffs:>8} {bram:>6} {dsp:>5} {status:>8} {is_winner:>8}"
        )
...
def main() -> None:
    ...
    if winner:
        ...
        print(f"\nNext step: ./build_opt_sweep.py {board_name} --skip-synth")
    else:
