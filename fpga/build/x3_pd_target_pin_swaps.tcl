...
# the recorded placement; mismatched implementations must fail before mutation.
namespace eval frost_x3_pd_target_pin_swaps {
    proc pin_map {cell} {
        ...
        foreach pin [lsort [get_pins -of_objects $cell -filter {DIRECTION == IN}]] {
            ...
            if {[llength $physical] != 1} {
                error "Nonunique physical input pin: $pin"
            }
    ...
    proc apply {audit_file} {
        ...
        set states {}
        # Validate both LUTs, including unused paired D5 BELs, before touching either.
        foreach recipe $recipes {
            lassign $recipe name ref init site old_map new_map
            set cell [get_cells -quiet $name]
            if {[llength $cell] != 1} {error "Expected one PD target LUT: $name"}
        ...
        set code [catch {
            ...
            foreach state $states recipe $recipes {
                ...
                set sibling_bel [get_bels -quiet [dict get $state LOC]/D5LUT]
                if {[llength [get_cells -quiet -of_objects $sibling_bel]] != 0} {
                    error "PD target pin refinement changed paired D5LUT occupancy"
                }
                lappend final_states $after
            }
        } message options]
        if {$code != 0} {
            # Never leave a partial pair applied if a Vivado command/check fails.
            set rollback_errors {}
            foreach state $states {
                if {[catch {
                    remap $state [dict get $state map]
                    reset_property LOCK_PINS [dict get $state cell]
                    set restored [snapshot [dict get $state cell]]
                    if {$restored ne $state} {error "Rollback state differs"}
                } rollback_error]} {
