...
def check_rat_lookup(result: LookupResult, expected: LookupResult, label: str) -> None:
    ...
    assert result.renamed == expected.renamed, (
...
async def exercise_fp_pending_done_repair(
    ...
    # They arrive in E1, where the repair-window block defers the dequeue; a
    # recovery hold keeps the repaired packet until dequeue can resume.
...
async def test_store_page_carry_pma_and_alignment_exact(dut: Any) -> None:
    ...
    # both carry directions; the wrapper's simulation-only full-address
    # reference checks the optimized bits on every live issue.
    alignment_cases = [
...
async def test_rs_full_stalls_dispatch(dut: Any) -> None:
    ...
    # Fill INT_RS to its configured depth.
...
async def test_rob_bypass_read_with_rs_state(dut: Any) -> None:
    ...
    # The arbiter selects the result combinationally, and the CDB register
    # delivers it to the ROB on the next rising edge.
            ...
            if issue["valid"]:
                if expected_issues:
                    ...
                    if model_issue is not None:
                        ...
                        assert issue["rob_tag"] in rs_live_tags, (
...
async def test_cdb_broadcast_wakes_all_rs_types(dut: Any) -> None:
    ...
    for rs_type in ALL_RS_TYPES:
        ...
        assert model_issue is not None
        assert issue["rob_tag"] == tags[rs_type], (
...
async def test_multi_fu_arbitration_contention(dut: Any) -> None:
    ...
    # Arbiter priority among them: FP_DIV (slot 6) > FP_MUL (5) > FP_ADD (4).
    ...
    for expected_tag in [tag_a, tag_b, tag_c]:
        commit = await wait_for_commit(dut_if)
        model.try_commit()
        assert commit["valid"] and commit["tag"] == expected_tag, (
...
async def test_mul_shim_end_to_end(dut: Any) -> None:
    ...
    # The multiplier takes 6 cycles at XLEN=64; wait for the CDB broadcast
...
async def test_integrated_fu_back_to_back(dut: Any) -> None:
    ...
    # Dispatch instruction A: MUL (6-cycle latency, issues first)
...
async def test_integrated_fu_flush_inflight(dut: Any) -> None:
    ...
    # Dispatch a MUL operation (6-cycle latency)
...
async def test_sq_commit_scan_flush_race_capture_then_kill(dut: Any) -> None:
    ...
    # a slow-tier write, which is what i_slow_write_inflight models below.
    ...
    for _ in range(10):
        ...
        await dut_if.step()
    w = dut_if.read_sq_mem_write()
    assert w["en"] and w["addr"] == addr_y and w["data"] == wbeat(data_y0), (
    ...
    # physical slot 1, which the per-entry peeks below use.
    s1_slot = 1
    for _ in range(5):
    ...
    assert (int(dut.u_sq.sq_addr_valid.value) >> s1_slot) & 1, (
        "S1 early address not delivered"
    )
    assert not ((int(dut.u_sq.sq_data_valid.value) >> s1_slot) & 1), (
    ...
    fwd = int(dut.sq_forward.value)
    assert not ((fwd >> 65) & 1) and not ((fwd >> 64) & 1), (
    ...
    assert int(dut.u_lq.sq_check_pending.value) == 1
    assert int(dut.sq_check_capture_valid.value) == 1, (
    ...
    assert (int(dut.u_sq.sq_valid.value) >> s1_slot) & 1, (
        "the dying store must still be scanned this cycle"
    )
    assert int(dut.u_sq.sq_forwarding_unit_inst.fwd_found_match.value) == 1
    assert int(dut.u_sq.sq_forwarding_unit_inst.fwd_can_fwd.value) == 1, (
        "the poisoned would-be-forward must be at the capture D-pins"
    )

    # ---------------------------------------------------------------------
    # Phase 6: edge E+1, where flush and capture land together.
    # ---------------------------------------------------------------------
    await dut_if.step()
    dut_if.clear_flush_all()

    fwd = int(dut.sq_forward.value)
    assert (fwd >> 65) & 1 and (fwd >> 64) & 1, (
        "poisoned capture must be LATCHED at E+1 (proves the window was hit)"
    )
    assert (fwd & 0xFFFF_FFFF) == data_stale, (
        f"captured forward data {fwd & 0xFFFFFFFF:#x} != dead store data"
    )
    ...
    assert int(dut.u_sq.sq_valid.value) == 0b11, (
...
async def test_lq_cdb_arbitration(dut: Any) -> None:
    ...
    # response at the next posedge (mem_outstanding=1); until its result
    # reaches the MEM slot, FP_ADD has the CDB to itself.
    ...
    assert cdb1.valid, "CDB should be valid (FP_ADD still driven)"
    assert cdb1.tag == tag_int, f"FP_ADD should be on lane 0, got tag={cdb1.tag}"
...
async def test_lr_sc_success_flow(dut: Any) -> None:
    ...
    # Drive done only once the write is in flight (write_inflight_cnt != 0)
...
async def test_amo_swap_integration(dut: Any) -> None:
    ...
    assert cdb.tag == tag
    assert cdb.value == expected_old_value, (
...
async def test_mmio_load_integration(dut: Any) -> None:
    ...
    # Let the LQ record the handoff before modeling the router's mandatory
    # pending stage: the router raises pending only for a handoff the LQ
    # already counts as outstanding (a wrapper integration invariant).
    await dut_if.step()
...
# =============================================================================
# SC Deadlock and Flush Hazard Tests
# =============================================================================


@cocotb.test()
async def test_sc_pending_does_not_block_older_load(dut: Any) -> None:
    """A pending SC must not block other MEM_RS issues.
    ...
    assert load_issued, (
        ...
        "(sc_pending does not gate MEM_RS issue)"
    )
...
FP_TWO_S = 0xFFFF_FFFF_4000_0000  # NaN-boxed 2.0f
FP_FOUR_S = 0xFFFF_FFFF_4080_0000  # NaN-boxed 4.0f


def _read_cdb_lanes(dut: Any) -> list[CdbBroadcast]:
    """Sample both combinational CDB broadcast lanes."""
    return [
...
async def _watch_stale(
    dut_if: TomasuloInterface,
    dead_tags: set[int],
    cycles: int,
    consumer_rs: int | None = None,
) -> None:
    """Assert no CDB broadcast of any dead tag, no consumer wakeup, no commit.

    Samples both CDB lanes every cycle for `cycles` cycles.  If consumer_rs is
    given, also asserts that RS never issues (its only entry waits on a
    producer that never completes, so an issue means a false CDB wakeup).
    """
    for cyc in range(cycles):
        await RisingEdge(dut_if.clock)
        await FallingEdge(dut_if.clock)
        for lane, cdb in enumerate(_read_cdb_lanes(dut_if.dut)):
            assert not (cdb.valid and cdb.tag in dead_tags), (
                f"STALE CDB delivery: lane{lane} broadcast tag={cdb.tag} "
                f"value=0x{cdb.value:x} {cyc} cycles into the post-flush window"
            )
        if consumer_rs is not None:
            issue = dut_if.read_rs_issue_for(consumer_rs)
            assert not issue["valid"], (
                f"False wakeup: consumer RS issued rob_tag={issue['rob_tag']} "
                f"{cyc} cycles into the post-flush window (its source producer "
                f"never completed, so only a stale broadcast can have woken it)"
            )
        commit = dut_if.read_commit()
        assert not commit["valid"], (
            f"Unexpected commit tag={commit['tag']} {cyc} cycles into the "
            f"post-flush window (head is parked on an incomplete anchor; a "
            f"commit means a stale delivery marked an entry done)"
        )


async def _stale_probe_drain_and_check_alive(
    dut_if: TomasuloInterface,
    anchor_tag: int,
    blocker_tag: int,
        ...
        for _ in range(cycles):
            ...
            for commit in (dut_if.read_commit(), dut_if.read_commit_2()):
                if commit["valid"]:
                    seen += 1
                    cocotb.log.info(
                        f"drain: commit tag={commit['tag']} value=0x{commit['value']:x}"
                    )
        return seen

    commits = 0
    for tag in (anchor_tag, blocker_tag):
        dut_if.drive_fu_complete(FU_FP_ADD, tag=tag, value=0x1234)
        commits += await count_commits(1)
        dut_if.clear_fu_complete(FU_FP_ADD)
        commits += await count_commits(1)
    ...
    assert commits >= expected_commits, (
        f"Pipeline did not drain after releasing the anchor: "
        f"{commits}/{expected_commits} commits"
    )
...
async def _run_fp_stale_probe(
    ...
    cp_id = dut_if.checkpoint_alloc_id
    tag_br = await dut_if.dispatch(
        make_branch_req(pc=0x1008, predicted_taken=True, predicted_target=0x3000),
    ...
    assert reused_tag == min(dead_tags), (
        f"probe expects tag reuse at tail rewind: got {reused_tag}, "
        f"expected {min(dead_tags)}"
    )
    dut_if.set_fu_ready(RS_INT, True)
    dut_if.drive_rs_dispatch(
        rs_type=RS_INT,
...
async def test_mem_single_delivery_misalign_collision(dut: Any) -> None:
    ...
    misalign_seen = 0
    for wake_delta in [-3, -2, -1, 0, 1]:
        cocotb.log.info(f"--- wake_delta={wake_delta} ---")
        dut_if, _model = await setup_test(dut)
        dut_if.dut.i_trap_misaligned_accesses.value = 1
        dut_if.set_fu_ready(RS_MEM, True)

        # Load at the ROB head: its first CDB delivery commits it via the
        # head-done bypass, so a duplicate delivery hits a freed entry.
        load_tag = await dut_if.dispatch(make_int_req(pc=0x2000, rd=5))
        blocker_tag = await dut_if.dispatch(make_int_req(pc=0x2004, rd=6))
        store_tag = await dut_if.dispatch(make_store_req(pc=0x2008))
        ...
        await dut_if.step()
        # Misaligned SW (addr = 0x3002, word store): src1 pending on the
        # blocker so the issue timing is controlled by the wake below.
        dut_if.drive_rs_dispatch(
            rs_type=RS_MEM,
            rob_tag=store_tag,
            op=OP_SW,
        ...
        # Let the load issue and reach the memory request.
        mem_req = {"en": False}
        for _ in range(8):
            mem_req = dut_if.read_lq_mem_request()
            if mem_req["en"]:
        ...
        # Wake the store wake_delta cycles relative to the memory response,
        # so one sweep iteration lands its misalign-issue cycle exactly on
        # the load-completion grant cycle.  Negative deltas wake before the
        # response; non-negative deltas wake inside the counted window.
        if wake_delta < 0:
            dut_if.drive_fu_complete(FU_FP_ADD, tag=blocker_tag, value=0x3000)
            await dut_if.step()
            dut_if.clear_fu_complete(FU_FP_ADD)
            for _ in range(-wake_delta - 1):
                await dut_if.step()

        # Single per-cycle window: drive the response at index 0, perform a
        # delayed wake at index wake_delta, and count every CDB delivery.
        load_broadcasts = 0
        dut_if.drive_lq_mem_response(0xDEAD_BEEF)
        for idx in range(16):
            if wake_delta >= 0:
                if idx == wake_delta:
                    dut_if.drive_fu_complete(FU_FP_ADD, tag=blocker_tag, value=0x3000)
                elif idx == wake_delta + 1:
                    dut_if.clear_fu_complete(FU_FP_ADD)
            for cdb in _read_cdb_lanes(dut_if.dut):
                if cdb.valid and cdb.tag == load_tag:
                    load_broadcasts += 1
            ...
            if idx == 0:
                dut_if.clear_lq_mem_response()
        assert load_broadcasts == 1, (
            f"load tag {load_tag} broadcast {load_broadcasts} times "
            f"(wake_delta={wake_delta}); MEM completions must deliver "
            f"exactly once"
        )
    assert misalign_seen >= 1, (
        "sweep never produced the misaligned-store exception delivery: "
        "collision alignment lost, tighten wake_delta sweep"
    )
    cocotb.log.info("=== Test Passed ===")
...
async def test_stale_cdb_fdiv_full_flush_probe(dut: Any) -> None:
    ...
    for flush_delay, contend in [
        ...
        tag_a = await dut_if.dispatch(make_int_req(pc=0x1000, rd=5))
        filler_a = filler_b = -1  # only used (and dispatched) when contending
        if contend:
        ...
        await Timer(1, unit="ps")
        new_blocker = await dut_if.dispatch(make_int_req(pc=0x4000, rd=6))
        new_consumer = await dut_if.dispatch(make_int_req(pc=0x4004, rd=7))
        assert new_blocker == tag_a, (
            f"full flush should rewind tail to head: got {new_blocker}, "
            f"expected {tag_a}"
        )
        assert new_consumer in dead_tags, "consumer must reuse a dead index"
        dut_if.set_fu_ready(RS_INT, True)
        dut_if.drive_rs_dispatch(
            rs_type=RS_INT,
            rob_tag=new_consumer,
            op=OP_ADD,
            src1_ready=False,
            src1_tag=new_blocker,
            src2_ready=True,
            src2_value=0x10,
            src3_ready=True,
        )
...
async def test_stale_cdb_fdiv_contention_partial_flush_probe(dut: Any) -> None:
    ...
    for flush_delay in [33, 35, 37, 39, 42]:
        ...
        filler_a = await dut_if.dispatch(make_int_req(pc=0x1004, rd=8))
        filler_b = await dut_if.dispatch(make_int_req(pc=0x1008, rd=9))
        blocker_tag = await dut_if.dispatch(make_int_req(pc=0x100C, rd=6))

        cp_id = dut_if.checkpoint_alloc_id
        tag_br = await dut_if.dispatch(
            make_branch_req(pc=0x1010, predicted_taken=True, predicted_target=0x3000),
            checkpoint_save=True,
        )
        ...
        # Hold contention a few more cycles across the flush, then release.
        for _ in range(4):
            await dut_if.step()
        dut_if.clear_fu_complete(FU_ALU)
        dut_if.clear_fu_complete(FU_MUL)
...
async def test_lq_no_ghost_alloc_during_partial_flush(dut: Any) -> None:
    ...
    # partial-flush pulse, carrying the tag a same-cycle dispatch would carry
    # (the pre-flush ROB tail).  The ROB rejects this alloc, and the LQ must
    # too.
...
async def test_lq_sq_alloc_during_full_flush_ignored(dut: Any) -> None:
    ...
    # Pre-populate one load + one store, then raise flush_all in the same
    # cycle as a fresh load alloc, and again with a fresh store alloc.
    tag_l0 = await dut_if.dispatch(make_int_req(pc=0x1000, rd=5))
    dut_if.drive_rs_dispatch(**_pending_load_kwargs(tag_l0))
    await dut_if.step()
    dut_if.clear_rs_dispatch()
    ...
    assert dut_if.sq_count == 0, (
        f"sq_count={dut_if.sq_count}: flush_all must squash the coincident store alloc"
    )
    cocotb.log.info("=== Test Passed ===")
...
async def _exercise_translated_store_kill_capture(
    ...
    # makes the next rising edge capture the raw payload in the same cycle
    # the recovery kills dmmu_out_valid.
    for _ in range(10):
        await Timer(1, unit="ps")
        ...
        raise TimeoutError("translated store never reached the DMMU S2 capture stage")

    assert int(dut.dmmu_out_valid.value), (
    ...
    await Timer(1, unit="ps")
    assert int(dut.dmmu_out_sq_capture_valid.value), (
        "recovery masked raw SQ payload capture"
    )
    assert not int(dut.dmmu_out_valid.value), "recovery did not kill dmmu_out_valid"
    assert int(dut.sq_addr_update_capture_valid.value), (
        "SQ did not take the raw payload capture"
    )
