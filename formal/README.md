...
| `ras_checkpoint` | The return-address stack's next pointer and count equal the reference equations, and every entry write matches the reference: a push or swap writes its link address where the reference puts it, a restore writes the saved top entry back, and no other entry changes |
