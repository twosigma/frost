...
/*
  ...
  Multi-cycle implementation (15-cycle latency, non-pipelined):
    Cycle 0: Capture operands
    Cycle 1: Unpack operands, detect special cases
    Cycle 2: Multiply mantissas (24x24 -> 48 bits)
    Cycle 2B: TIMING: Extra pipeline stage after DSP multiply
...
module fp_fma #(
  ...
  typedef enum logic [4:0] {
    ...
    STAGE1  = 5'b00001,
    STAGE2  = 5'b00010,
    STAGE2B = 5'b10000,  // TIMING: Extra pipeline stage after DSP multiply
    STAGE3A = 5'b00011,
    STAGE3B = 5'b00100,
    STAGE4  = 5'b00101,
    STAGE4B = 5'b00110,
  ...
  // =========================================================================
  // Stage 2 -> Stage 2B Pipeline Registers (TIMING: break DSP critical path)
  // =========================================================================
  // Adding an extra pipeline stage after the DSP multiply allows the synthesis
  // tool to use DSP48 internal MREG register and improves placement flexibility.

  logic        [   ProdBits-1:0] prod_mant_s2b;
  logic signed [ ExpExtBits-1:0] prod_exp_s2b;
  logic                          prod_sign_s2b;
  logic signed [ ExpExtBits-1:0] c_exp_s2b;
  logic        [   MantBits-1:0] mant_c_s2b;
  logic                          c_sign_s2b;
  logic        [            2:0] rm_s2b;
  logic                          is_special_s2b;
  logic        [   FP_WIDTH-1:0] special_result_s2b;
  logic                          special_invalid_s2b;
  ...
  always_ff @(posedge i_clk) begin
    if (i_rst) begin
      ...
      rm_s2b <= 3'b0;
      is_special_s2b <= 1'b0;
      special_result_s2b <= '0;
      special_invalid_s2b <= 1'b0;
      // Stage 3 (before LZC)
      prod_mant_s3 <= '0;
      prod_exp_s3 <= '0;
      prod_sign_s3 <= 1'b0;
      ...
      case (state)
        ...
        STAGE2: begin
          // TIMING: Capture DSP multiply result into s2b registers (extra pipeline stage)
          prod_mant_s2b <= prod_mant_s2_comb;
          prod_exp_s2b <= prod_exp_s2;
          prod_sign_s2b <= prod_sign_s2;
          c_exp_s2b <= c_exp_s2;
          mant_c_s2b <= mant_c_s2;
          c_sign_s2b <= c_sign_s2;
          rm_s2b <= rm_s2;
          is_special_s2b <= is_special_s2;
          special_result_s2b <= special_result_s2;
          special_invalid_s2b <= special_invalid_s2;
        end

        STAGE2B: begin
          // TIMING: Copy s2b registers to s3 registers (breaks DSP critical path)
          prod_mant_s3 <= prod_mant_s2b;
          prod_exp_s3 <= prod_exp_s2b;
          prod_sign_s3 <= prod_sign_s2b;
          c_exp_s3 <= c_exp_s2b;
          mant_c_s3 <= mant_c_s2b;
          c_sign_s3 <= c_sign_s2b;
          rm_s3 <= rm_s2b;
          is_special_s3 <= is_special_s2b;
          special_result_s3 <= special_result_s2b;
          special_invalid_s3 <= special_invalid_s2b;
        end
