...
module fp_adder #(
  ...
  logic        [ LzcBits-1:0] shift_amt_s5a;
  logic signed [ExpExtBits:0] shift_amt_signed_s5a;
  ...
  always_comb begin
    ...
    shift_amt_s5a = '0;
    shift_amt_signed_s5a = '0;

    if (normalized_exp_s5 <= 0) begin
      shift_amt_signed_s5a = $signed({1'b0, {ExpExtBits{1'b0}}}) + 1 -
          $signed({normalized_exp_s5[ExpExtBits-1], normalized_exp_s5});
      if (shift_amt_signed_s5a >= MantBitsPlus3Signed) shift_amt_s5a = MantBitsPlus3Shift;
      else shift_amt_s5a = shift_amt_signed_s5a[LzcBits-1:0];
      if (shift_amt_s5a >= MantBitsPlus3Shift) begin
        shifted_ext_s5a = '0;
        shifted_sticky_s5a = |mantissa_ext_s5a;
      end else if (shift_amt_s5a != 0) begin
        shifted_ext_s5a = mantissa_ext_s5a >> shift_amt_s5a;
        shifted_sticky_s5a = 1'b0;
        for (int i = 0; i < (MantBits + 3); i++) begin
          if (i < shift_amt_s5a) shifted_sticky_s5a = shifted_sticky_s5a | mantissa_ext_s5a[i];
        end
      end
