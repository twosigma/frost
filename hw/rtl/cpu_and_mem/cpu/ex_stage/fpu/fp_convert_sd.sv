...
module fp_convert_sd #(
  ...
  always_comb begin
    mant_norm_d = {1'b1, frac_d};
    exp_unbiased_d = $signed({2'b0, exp_d}) - 13'sd1023;
    if (exp_d == 11'b0) begin
      if (frac_d != 52'b0) begin
        mant_norm_d = ({1'b0, frac_d} << (lzc_d + 1'b1));
        exp_unbiased_d = -13'sd1022 - $signed({7'b0, lzc_d}) - 13'sd1;
      end else begin
        mant_norm_d = 53'b0;
