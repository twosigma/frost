...
| `cached_tier_adapter` | `memory_if/` | Word↔line adapter between the router and the cache hierarchy (`lib/cache/frost_cache_hierarchy`): converts CPU words to 32 B line transactions, serializes one in flight, and presents read-valid / write-done / write-inflight back to the router. |
