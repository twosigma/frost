...
| `perf_counter_aggregator` | `perf/` | Accumulates the 42 top-level and 24 cache-hierarchy performance counters, snapshots them alongside the 64 wrapper-owned counters, keeps the previous cache snapshot for deferred software readback, and muxes the three-block global index space onto the CSR read port. Present only when `cpu_ooo`'s `PERF_COUNTERS` parameter is 1; the production build leaves the counters out. Counter definitions and the build option are in [`perf/README.md`](cpu_ooo/perf/README.md). |
| `branch_resolution` | `branch_recovery/` | Resolves the registered branch/JAL/JALR payload from INT_RS (wrapping `branch_jump_unit`) in parallel with flush and checkpoint-owner validation, and applies that validation only at the architectural ROB `branch_update`. A direct branch's target check is the one-bit compare ID made against the prediction; JALR compares its computed target against the INT station's tag-indexed `predicted_target`. Simulation asserts that each side-RAM word it consumes agrees with the packet beside it: the direct branch's one-bit check against the full target compare, and JALR's row `link_addr` against both the payload's `imm` and the row's own `pc`. |
