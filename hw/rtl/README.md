...
| `cpu_and_mem/imem_predecode.sv` | In use | Instruction RAM with 64-bit fetch (even/odd interleaved BRAM banks, each resource-neutrally split into 28 cold data bits plus the frontend-hot word bits `{15,10,7,6}`), word-local class/bundle and RVC source-hot predecode sideband, a seven-bit narrow replica for high-allows and other hot fields, plus protected helper-isolated 32Kx4 PC-metadata banks. The even payload is `{pairable-native-hi,pairable-compressed-hi,compressed-hi,compressed-lo}`; the odd payload repacks dead BRAM lane 3 as the live `Slot2StartValidLo` source while its public pairable-native-high stays on a scalar helper. Metadata lanes 0–1, odd lane 3 as Slot2StartValidLo, and the dedicated even Slot2 replica launch from BRAM. Public metadata lanes 2–3 and both parity banks' `EvenLocalPairValid` and `PairableNativeLo` predicates launch from scalar LUTRAM plus output FF. Canonical compressed/sideband and repacked PC-metadata lanes remain same-edge equivalence oracles for the scalar copies. |
