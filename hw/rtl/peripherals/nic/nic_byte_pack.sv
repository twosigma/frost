...
module nic_byte_pack #(
  ...
  logic [63:0] beat_data_q[2];
  logic [3:0] beat_bytes_q[2];
  logic beat_last_q[2];
  logic [63:0] beat_data;
  logic [3:0] beat_bytes;
  logic beat_last, beat_push, beat_fire;
  assign beat_data = beat_data_q[beat_rd_q];
  assign beat_bytes = beat_bytes_q[beat_rd_q];
  assign beat_last = beat_last_q[beat_rd_q];
  assign o_beat_ready = accepting_q && !beat_count_q[1];
  assign beat_push = i_beat_valid && o_beat_ready;

  always_ff @(posedge i_clk) begin
    if (i_rst || i_flush || i_start) begin
      accepting_q <= !i_rst && !i_flush && i_start;
      beat_count_q <= '0;
      beat_rd_q <= 1'b0;
      beat_wr_q <= 1'b0;
    end else begin
      beat_count_q <= beat_count_q + 2'(beat_push) - 2'(beat_fire);
      if (beat_push) begin
        beat_data_q[beat_wr_q] <= i_beat_data;
        beat_bytes_q[beat_wr_q] <= i_beat_bytes;
        beat_last_q[beat_wr_q] <= i_beat_last;
        beat_wr_q <= !beat_wr_q;
        if (i_beat_last) accepting_q <= 1'b0;
      end
      if (beat_fire) beat_rd_q <= !beat_rd_q;
    end
  end
  ...
  always_comb begin
    ...
    for (int j = 0; j < 8; j++) begin
      if ((j < int'(beat_bytes)) && (16'(j) < room)) begin
        rot16[int'(rot_q)+j]    = beat_data[j*8+:8];
  ...
  always_comb begin
    bytes_kept_now = '0;
    for (int j = 0; j < 8; j++) begin
      if ((j < int'(beat_bytes)) && (16'(j) < room)) bytes_kept_now = 4'(j + 1);
    end
  end
  ...
  // A queued beat is placed when the packer is active, not flushing, and the lower
